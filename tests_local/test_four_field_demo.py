...
import jinja2
import pytest
from tools import demo_four_field_episode_outline as demo
from service.drama_by_creativity import stage_episode_planning as planning


def episode(i):
    return dict(episode_id=i, title='标题', core_plot='剧情', roles=['主角'])


@pytest.mark.parametrize('kind', ['extra', 'missing', 'id', 'empty'])
def test_rejects_invalid_four_field_output(kind):
    ep = episode(1)
    if kind == 'extra': ep['ending_hook'] = '不要这个字段'
    if kind == 'missing': del ep['title']
    if kind == 'id': ep['episode_id'] = True
    if kind == 'empty': ep['roles'] = []
    with pytest.raises(ValueError): demo.validate_four_fields([ep], 1, 1)


def test_four_fields_through_stages_and_three_rewrites(tmp_path, monkeypatch):
    ...
    async def infer(**kwargs):
        ...
        assert '"highlights"' not in rendered and '"ending_hook"' not in rendered
        if 'Original' in p:
            original = json.loads(p['Original'])
            demo.validate_four_fields(original, 1, 4)
            if 'Review' in p:
                rewrites.append(p)
    ...
    with patch.object(planning,'llm_inference_json',side_effect=infer), patch.object(planning,'make_outline_llm',side_effect=lambda text:SimpleNamespace(template=jinja2.Template(text))):
        result = asyncio.run(demo.generate_four_fields(None, story, 4, {}, 'unused'))
    assert len(reviews) == 4 and len(rewrites) == 3
    assert result[0]['core_plot'] == '剧情改改改'
    demo.validate_four_fields(result, 1, 4)
...
def test_demo_skips_polishing_and_passes_original_story(tmp_path):
    ...
    with patch.object(demo.base, 'generate', side_effect=AssertionError('不得调用润色流程')), \
         patch.object(demo, 'generate_four_fields', side_effect=downstream) as generate:
