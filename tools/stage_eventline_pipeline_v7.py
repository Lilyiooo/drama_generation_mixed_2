#!/usr/bin/env python3
"""多 stage 串联流水线：依次对 outline 中各 stage 执行 v7 树搜索，逐段替换 event_list。"""
...
V7_SCRIPT = PROJECT_ROOT / "tools/stage_eventline_local_tree_v7.py"
DEFAULT_OUTLINE = PROJECT_ROOT / "output/60ep_0810_1946/03_story_outline/outline.json"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_pipeline_v7"
...
def run_v7_for_stage(
    *,
    ...
    """运行 v7 为指定 stage 生成 final_event_line，返回 (事件列表, 输出路径, trope_bank)。"""
    timestamp = datetime.now().strftime("%m%d_%H%M%S")
    safe_stage = stage.replace("/", "_").replace(" ", "_")
    v7_output = output_dir / "v7_results" / f"{safe_stage}_{timestamp}.json"
    v7_output.parent.mkdir(parents=True, exist_ok=True)
    ...
    cmd = [
        ...
        "--output", str(v7_output),
    ]
    if initial_trope_bank:
        bank_path = v7_output.parent / f"{safe_stage}_{timestamp}_bank.json"
    ...
    # 读取结果
    v7_data = load_json(v7_output)
    final_event_line = v7_data.get("final_event_line", [])
    trope_bank = v7_data.get("trope_bank", [])
    if not final_event_line:
        raise RuntimeError(f"v7 没有生成 final_event_line，请检查 {v7_output}")

    print(f"✅ {stage} 完成: 生成 {len(final_event_line)} 个事件，bank {len(trope_bank)} 条")
    return final_event_line, v7_output, trope_bank


def main() -> None:
    parser = argparse.ArgumentParser(
        description="多 stage 串联流水线：依次执行 v7 树搜索并逐段替换 outline"
    )
    parser.add_argument(
        "--outline-path",
        default=None,
    ...
    if explicit_outline:
        ...
        # 自动续传：优先找 outline_after_<stage>_*.json，其次复用 v7_results/<stage>_*.json
        ...
        if after_candidates:
            ...
            v7_candidates = sorted(
                output_dir.glob("v7_results/*.json"),
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if v7_candidates:
                latest = v7_candidates[0]
                done_stage = parse_stage_from_name(latest.name, prefix="")
                v7_data = load_json(latest)
                fel = v7_data.get("final_event_line", [])
                if fel:
                    current_outline = load_json(DEFAULT_OUTLINE)
                    current_outline = replace_stage_events(current_outline, done_stage, fel)
                    print(f"🔁 复用已有 v7 结果: {latest.name}（{done_stage}，{len(fel)} 事件）")
                    stages, _ = drop_completed_stages(stages, done_stage)
    ...
    for idx, stage in enumerate(stages, 1):
        ...
        # 保存当前 outline 供 v7 读取
        stage_input = output_dir / f"outline_before_{stage}_{timestamp}.json"
        save_json(stage_input, current_outline)

        # 运行 v7（把前序 stage 已建好的 bank 传下去，避免重复建库）
        final_event_line, v7_output_path, trope_bank = run_v7_for_stage(
