...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_local_tree_v3"
...
- 说明：虽然还没直接碰到锚点，但它明显把局面推向“尚书不得不出面”，而且推进机制从公开闹场切到设局智取，适合作为下一轮的强承接点。
- 链 B：coherence = 7, novelty = 3, target_fit = 8, continuation_value = 5
- 说明：它确实接触到了目标锚点，但推进方式仍是重复既有套路，新意和后续展开空间都偏弱。
...
- novelty 不是猎奇，而是相对于 fixed_prefix 已出现的推进套路，是否出现了真实的新推进机制。
...
2. `novelty`：1-10，评价它相对 fixed_prefix 的已有推进套路，是否有真实的新推进机制。
...
def compute_local_chain_score(score_payload: dict[str, Any], anchor_reached: bool) -> float:
    ...
    return round(0.32 * coherence + 0.22 * novelty + 0.26 * target_fit + 0.20 * continuation_value + reached_bonus, 4)
...
def build_output_path(output_dir: str | Path, stage_name: str) -> Path:
    ...
    return Path(output_dir).expanduser().resolve() / f"{safe_stage}_local_tree_v3_{timestamp}.json"
...
def assemble_generation_result(
    *,
    ...
    return {
        "meta": {
            "mode": "local_tree_search_experiment_v3",
...
def generate_stage_eventline_with_local_tree_v3(
    *,
    outline_path: str | Path,
    stage_name: str,
    branch_factor: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对指定 stage 执行局部树搜索式事件线生成实验 v3")
    ...
    parser.add_argument("--output", help="输出 JSON 路径；不传则自动落到 output/stage_eventline_local_tree_v3")
...
def main() -> None:
    ...
    result = generate_stage_eventline_with_local_tree_v3(
    ...
    print(f"已完成 `{args.stage}` stage 的局部树搜索实验 v3")
