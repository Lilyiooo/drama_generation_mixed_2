...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_local_tree_v8"
    ...
    for attempt in range(3):
        ...
        try:
            ...
            progress_logger.log(
                ...
                total_anchors=total_anchors,
                phase="候选评分重试",
                detail=f"attempt={attempt + 1}/3 | {error}",
            )
            if attempt < 2:
                time.sleep(1.0 * (attempt + 1))

    raise RuntimeError(f"候选评分重试 3 次仍失败：{last_error}")
...
def assemble_generation_result(
    *,
    ...
    return {
        "meta": {
            "mode": "local_tree_search_experiment_v8",
...
def generate_stage_eventline_with_local_tree_v8(
    *,
    outline_path: str | Path,
    stage_name: str,
    branch_factor: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对指定 stage 执行局部树搜索式事件线生成实验 v8")
    ...
    parser.add_argument("--output", help="输出 JSON 路径；不传则自动落到 output/stage_eventline_local_tree_v8")
    parser.add_argument(
        "--initial-trope-bank",
...
def main() -> None:
    ...
    result = generate_stage_eventline_with_local_tree_v8(
    ...
    print(f"已完成 `{args.stage}` stage 的局部树搜索实验 v8")
