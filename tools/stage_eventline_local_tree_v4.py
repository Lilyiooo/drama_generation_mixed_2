...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_local_tree_v4"
...
2. `novelty`：1-10，评价它相对 fixed_prefix 的已有推进套路，是否有真实的新推进机制，而非复用已固定的完整前缀事件中的叙事套路（包含前序 stage 与本 stage 已完成部分）。
...
def build_output_path(output_dir: str | Path, stage_name: str) -> Path:
    ...
    return Path(output_dir).expanduser().resolve() / f"{safe_stage}_local_tree_v4_{timestamp}.json"
...
def assemble_generation_result(
    *,
    ...
    return {
        "meta": {
            "mode": "local_tree_search_experiment_v4",
...
def generate_stage_eventline_with_local_tree_v4(
    *,
    outline_path: str | Path,
    stage_name: str,
    branch_factor: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对指定 stage 执行局部树搜索式事件线生成实验 v4")
    ...
    parser.add_argument("--output", help="输出 JSON 路径；不传则自动落到 output/stage_eventline_local_tree_v4")
...
def main() -> None:
    ...
    result = generate_stage_eventline_with_local_tree_v4(
    ...
    print(f"已完成 `{args.stage}` stage 的局部树搜索实验 v4")
