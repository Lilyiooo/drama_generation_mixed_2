...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_local_tree_v5"
...
def build_output_path(output_dir: str | Path, stage_name: str) -> Path:
    ...
    return Path(output_dir).expanduser().resolve() / f"{safe_stage}_local_tree_v5_{timestamp}.json"
...
def assemble_generation_result(
    *,
    ...
    return {
        "meta": {
            "mode": "local_tree_search_experiment_v5",
...
def generate_stage_eventline_with_local_tree_v5(
    *,
    outline_path: str | Path,
    stage_name: str,
    branch_factor: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对指定 stage 执行局部树搜索式事件线生成实验 v5")
    ...
    parser.add_argument("--output", help="输出 JSON 路径；不传则自动落到 output/stage_eventline_local_tree_v5")
...
def main() -> None:
    ...
    result = generate_stage_eventline_with_local_tree_v5(
    ...
    print(f"已完成 `{args.stage}` stage 的局部树搜索实验 v5")
