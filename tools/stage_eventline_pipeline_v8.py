#!/usr/bin/env python3
"""多 stage 串联流水线：依次对 outline 中各 stage 执行 v8 树搜索，逐段替换 event_list。"""
...
def run_v8_for_stage(
    *,
    ...
    """运行 v8 为指定 stage 生成 final_event_line，返回 (事件列表, 输出路径, trope_bank)。"""
    timestamp = datetime.now().strftime("%m%d_%H%M%S")
    safe_stage = stage.replace("/", "_").replace(" ", "_")
    v8_output = output_dir / "v8_results" / f"{safe_stage}_{timestamp}.json"
    v8_output.parent.mkdir(parents=True, exist_ok=True)
    ...
    cmd = [
        ...
        "--output", str(v8_output),
    ]
    if initial_trope_bank:
        bank_path = v8_output.parent / f"{safe_stage}_{timestamp}_bank.json"
    ...
    # 读取结果
    v8_data = load_json(v8_output)
    final_event_line = v8_data.get("final_event_line", [])
    trope_bank = v8_data.get("trope_bank", [])
    if not final_event_line:
        raise RuntimeError(f"v8 没有生成 final_event_line，请检查 {v8_output}")

    print(f"✅ {stage} 完成: 生成 {len(final_event_line)} 个事件，bank {len(trope_bank)} 条")
    return final_event_line, v8_output, trope_bank


def main() -> None:
    parser = argparse.ArgumentParser(
        description="多 stage 串联流水线：依次执行 v8 树搜索并逐段替换 outline"
    )
    parser.add_argument(
        "--outline-path",
        default=None,
    ...
    if explicit_outline:
        ...
        # 自动续传：优先找 outline_after_<stage>_*.json，其次复用 v8_results/<stage>_*.json
        ...
        if after_candidates:
            ...
            v8_candidates = sorted(
                output_dir.glob("v8_results/*.json"),
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if v8_candidates:
                latest = v8_candidates[0]
                done_stage = parse_stage_from_name(latest.name, prefix="")
                v8_data = load_json(latest)
                fel = v8_data.get("final_event_line", [])
                if fel:
                    current_outline = load_json(DEFAULT_OUTLINE)
                    current_outline = replace_stage_events(current_outline, done_stage, fel)
                    print(f"🔁 复用已有 v8 结果: {latest.name}（{done_stage}，{len(fel)} 事件）")
                    stages, _ = drop_completed_stages(stages, done_stage)
    ...
    for idx, stage in enumerate(stages, 1):
        ...
        # 保存当前 outline 供 v8 读取
        stage_input = output_dir / f"outline_before_{stage}_{timestamp}.json"
        save_json(stage_input, current_outline)

        # 运行 v8（把前序 stage 已建好的 bank 传下去，避免重复建库）
        final_event_line, v8_output_path, trope_bank = run_v8_for_stage(
