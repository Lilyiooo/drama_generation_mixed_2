...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_local_tree_v7"
...
2. `novelty`：1-10，评价它相对"已用叙事套路 bank"中记录的因果链（起因->手段->结果->反应），是否引入了新的因果推进模式，而非复用已固定的完整前缀事件中的叙事套路（包含前序 stage 与本 stage 已完成部分）；若本条链的核心套路与 bank 中某条高度雷同或相似度较高（换皮复用），novelty 必须给低分。局部事件链内部的因果重复也应当适度扣分。
3. `target_fit`：1-10，若已到锚点，评价到达方式是否自然 以及是否真的基本实现了锚点所要求的剧情；若未到锚点，评价它是否实质性逼近目标而不是原地打转。不要只因为它自我判定到了锚点就给高分，需要关注的是持续推进的剧情质量，因为即使没到，之后也可以往后继续生成的。
...
def build_output_path(output_dir: str | Path, stage_name: str) -> Path:
    ...
    return Path(output_dir).expanduser().resolve() / f"{safe_stage}_local_tree_v7_{timestamp}.json"
...
def assemble_generation_result(
    *,
    ...
    return {
        "meta": {
            "mode": "local_tree_search_experiment_v7",
...
def generate_stage_eventline_with_local_tree_v7(
    *,
    outline_path: str | Path,
    stage_name: str,
    branch_factor: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对指定 stage 执行局部树搜索式事件线生成实验 v7")
    ...
    parser.add_argument("--output", help="输出 JSON 路径；不传则自动落到 output/stage_eventline_local_tree_v7")
    parser.add_argument(
        "--initial-trope-bank",
...
def main() -> None:
    ...
    result = generate_stage_eventline_with_local_tree_v7(
    ...
    print(f"已完成 `{args.stage}` stage 的局部树搜索实验 v7")
