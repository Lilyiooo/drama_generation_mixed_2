#!/usr/bin/env python3
"""Matched direct-outline baseline for the three completed upstream candidate runs."""
...
SOURCE_ROOT = SCRIPT_ROOT / "output"
ROOT = SCRIPT_ROOT / "output/new_scriptdrama_matched_direct_history_baseline_v1"
...
RUN_SEEDS = {"R01": 362100, "R02": 362200, "R03": 362300}
...
PROMPT_VERSION = "new-scriptdrama-matched-outline-direct-all-history-v1"
...
def source_run_dir(run: str) -> Path:
    return SOURCE_ROOT / f"new_scriptdrama_structured_hybrid_gate_{run}"


def source_asset_path(run: str) -> Path:
    return source_run_dir(run) / "generation_assets.json"


def frozen_asset_path(run: str) -> Path:
    return ROOT / "frozen_sources" / run / "generation_assets.json"


def validate_assets(assets: dict) -> None:
    if not isinstance(assets.get("world_view"), str):
        raise ValueError("Invalid generation asset: world_view must be a string")
    for key in ("role_info", "story_outline"):
...
def prepare() -> None:
    paths = {run: source_asset_path(run) for run in RUNS}
    missing = [str(path) for path in paths.values() if not path.is_file()]
    if missing:
        raise FileNotFoundError("Missing source assets: " + ", ".join(missing))
    assets_by_run = {run: read_json(path) for run, path in paths.items()}
    hashes = {run: sha_bytes(paths[run].read_bytes()) for run in RUNS}
    for run, assets in assets_by_run.items():
        validate_assets(assets)
        expected = {f"episode_{episode:02d}.json" for episode in range(1, EPISODES + 1)}
        actual = {path.name for path in (source_run_dir(run) / "05_drama").glob("episode_*.json")}
        if actual != expected:
            raise ValueError(
                f"Candidate {run} must contain exactly {EPISODES} episodes; "
    ...
    protocol = {
        ...
        "description": (
            "Per-run matched direct episode-outline baseline; every episode receives all prior scripts "
            "from its own baseline trajectory verbatim."
        ),
        "source_experiment": str(SCRIPT_ROOT),
        "paired_sources": {
            run: {
                "candidate_directory": str(source_run_dir(run)),
                "generation_assets": str(paths[run]),
                "generation_assets_sha256": hashes[run],
                "world_view_empty": not bool(assets_by_run[run]["world_view"].strip()),
            }
            for run in RUNS
        },
        "episodes": EPISODES,
    ...
    for run in RUNS:
        atomic_json(frozen_asset_path(run), assets_by_run[run])
    ...
    for run in RUNS:
        print(
            f"{run} asset_sha256={hashes[run]} "
            f"episodes={len(assets_by_run[run]['episode_outlines'])} "
            f"world_view_empty={not bool(assets_by_run[run]['world_view'].strip())}"
        )


def require_prepared(run: str) -> tuple[dict, dict, str]:
    protocol_path = ROOT / "protocol.json"
    assets_path = frozen_asset_path(run)
    ...
    source = source_asset_path(run)
    expected_hash = protocol["paired_sources"][run]["generation_assets_sha256"]
    if sha_bytes(source.read_bytes()) != expected_hash:
        raise ValueError(f"Candidate {run} generation assets changed after baseline preparation")
    if canonical_sha(assets) != canonical_sha(read_json(source)):
        raise ValueError(f"Frozen candidate assets no longer match live source for {run}")
...
def verify_existing_prefix(run: str, assets: dict, script_format: str) -> list[str]:
    ...
    for episode in range(1, EPISODES + 1):
        ...
        if output.is_file() and not metadata.exists():
            ...
            traces = [
                read_json(path)
                for path in sorted((ROOT / run / "attempts").glob(f"episode_{episode:02d}_attempt_*.json"))
                if read_json(path).get("status") == "success"
            ]
            matching = [trace for trace in traces if trace.get("response_sha256") == sha_text(content)]
            if not matching:
                raise ValueError(f"Cannot audit output-only checkpoint for {run}/E{episode:02d}")
            trace = matching[-1]
            ...
            atomic_json(metadata, {
                ...
                "attempts": trace.get("attempt"),
                "base_url": None,
                "recovered_output_only_checkpoint": True,
                "recovery_attempt_trace": trace,
            })
...
def generate_run(run: str, limit: int | None, base_url: str, counter: TokenCounter) -> dict:
    _, assets, script_format = require_prepared(run)
...
def evaluate_run(run: str, execute_api: bool) -> dict:
    ...
    if score_path.is_file():
        return {"run": run, "returncode": 0, "skipped": True, "score": str(score_path)}
...
def write_report() -> None:
    ...
    for run in RUNS:
        ...
        candidate_path = source_run_dir(run) / "drama_evaluations_logic_quality_qwen38_v24/full_60_episodes/scores.json"
        ...
        if baseline_payload.get("evaluation_mode") != candidate_payload.get("evaluation_mode"):
            raise ValueError(f"Evaluation mode mismatch for {run}")
        baseline = score_pair(baseline_path)
        candidate = score_pair(candidate_path)
        rows.append(
            {
                "run": run,
                "baseline": {"logic": baseline[0], "quality": baseline[1]},
                "candidate": {"logic": candidate[0], "quality": candidate[1]},
                "candidate_minus_baseline": {
                    "logic": round(candidate[0] - baseline[0], 4),
                    "quality": round(candidate[1] - baseline[1], 4),
                },
            }
        )
    baseline_logic = [row["baseline"]["logic"] for row in rows]
    baseline_quality = [row["baseline"]["quality"] for row in rows]
    candidate_logic = [row["candidate"]["logic"] for row in rows]
    candidate_quality = [row["candidate"]["quality"] for row in rows]
    ...
    report = {
        ...
        "comparison": "new upstream structured-hybrid-gate candidate minus matched direct-outline/all-history baseline",
        ...
        "summary": {
            ...
            "candidate_logic_mean": statistics.mean(candidate_logic),
            "candidate_logic_sample_std": statistics.stdev(candidate_logic),
            "candidate_quality_mean": statistics.mean(candidate_quality),
            "candidate_quality_sample_std": statistics.stdev(candidate_quality),
            ...
            "candidate_logic_wins": sum(value > 0 for value in deltas_logic),
            "candidate_quality_wins": sum(value > 0 for value in deltas_quality),
        },
    }
    target = ROOT / "reports/matched_baseline_vs_upstream_candidate_v24.json"
