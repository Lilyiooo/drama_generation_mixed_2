#!/usr/bin/env python3
"""多 stage 串联流水线：依次对 outline 中各 stage 执行 v5 树搜索，逐段替换 event_list。"""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "output/stage_eventline_pipeline_v5"
...
def run_v5_for_stage(
    *,
    ...
    """运行 v5 为指定 stage 生成 final_event_line，返回事件列表和输出 JSON 路径。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="多 stage 串联流水线：依次执行 v5 树搜索并逐段替换 outline"
    )
    parser.add_argument(
        "--outline-path",
        default=None,
    ...
    if explicit_outline:
        ...
        # 自动续传：优先找 outline_after_<stage>_*.json，其次复用 v5_results/<stage>_*.json
        ...
        if after_candidates:
            ...
            v5_candidates = sorted(
                output_dir.glob("v5_results/*.json"),
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if v5_candidates:
                latest = v5_candidates[0]
                done_stage = parse_stage_from_name(latest.name, prefix="")
                v5_data = load_json(latest)
                fel = v5_data.get("final_event_line", [])
                if fel:
                    current_outline = load_json(DEFAULT_OUTLINE)
                    current_outline = replace_stage_events(current_outline, done_stage, fel)
                    print(f"🔁 复用已有 v5 结果: {latest.name}（{done_stage}，{len(fel)} 事件）")
                    stages, _ = drop_completed_stages(stages, done_stage)
