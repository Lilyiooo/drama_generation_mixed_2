#!/usr/bin/env bash
# Usage: bash run_local_memory_variants.sh all_subtree|nmf_no_cards|all [evaluator options]
set -euo pipefail
...
case "${1:-}" in
 all)
    shift
    bash "$EVAL_DIR/run_local_memory_variants.sh" all_subtree "$@"
    exec bash "$EVAL_DIR/run_local_memory_variants.sh" nmf_no_cards "$@"
    ;;
 all_subtree)
    SOURCE_DIR="$SOURCE_ROOT/qwen36_27b_88_outline_pipeline_all_subtree_20260916_155153"
    ;;
 nmf_no_cards)
    SOURCE_DIR="$TASK_ROOT/drama-generation/outputs/qwen36_27b_88_outline_nmf_20260916_161207"
    ;;
 *) echo '用法：bash run_local_memory_variants.sh all_subtree|nmf_no_cards|all [--dry-run 等评测参数]' >&2; exit 2 ;;
esac
label="$1"
shift
