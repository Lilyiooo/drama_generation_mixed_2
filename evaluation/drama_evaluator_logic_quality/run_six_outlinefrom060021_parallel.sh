#!/usr/bin/env bash
# Evaluate the six scripts derived from shortdrama_full_20260919_060021.
# Rerunning without --restart reuses validated evaluation artifacts.
set -euo pipefail

EVAL_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE="/inspire/hdd/global_user/hesiyang-CZXS25210137"
OUTPUT_ROOT="$WORKSPACE/drama_operator_new_v9_future5_state2_complete/drama_operator_hybrid_state_pipeline_shortdrama/output"
PYTHON="${PYTHON_BIN:-$WORKSPACE/anaconda3/bin/python3}"
...
RESULT_ROOT="${DRAMA_EVAL_RESULT_ROOT:-$EVAL_DIR/output/six_outlinefrom060021_subdimension_fusion_20260920_$model_label}"
...
sources=(
  "$OUTPUT_ROOT/shortdrama_full_history_outlinefrom060021_20260919_084641"
  "$OUTPUT_ROOT/shortdrama_full_20260919_060021"
  "$OUTPUT_ROOT/shortdrama_baseline_1500_2000_outlinefrom060021_20260919_090316"
  "$OUTPUT_ROOT/shortdrama_memory_1500_2000_outlinefrom060021_20260919_090316"
  "$OUTPUT_ROOT/shortdrama_baseline_2000_2500_outlinefrom060021_20260919_124130"
  "$OUTPUT_ROOT/shortdrama_memory_2000_2500_outlinefrom060021_20260919_124130"
)
...
if [[ "$summarize_only" == false ]]; then
  # Export and validate all 60 episodes before making any model requests.
  for i in "${!names[@]}"; do
    "$PYTHON" "$EVAL_DIR/tools/export_pipeline_script.py" \
      "${sources[i]}" "$RESULT_ROOT/inputs/${names[i]}.txt"
  done

  printf '060021大纲衍生六组并行评测，单组内部并发为1。\n模型：%s\n结果目录：%s\n' \
  ...
  for i in "${!pids[@]}"; do
    ...
    if [[ "$dry_run" == false ]]; then
      printf '%s\n' "$code" > "$RESULT_ROOT/${names[i]}.exitcode"
    fi
    printf '%s 退出码：%s\n' "${names[i]}" "$code"
  done
  trap - INT TERM
...
rows, csv_rows = [], []
for name in names:
    exit_file = root / f"{name}.exitcode"
    result_file = root / name / "multi_agent_result.json"
    code = int(exit_file.read_text()) if exit_file.exists() else 1
