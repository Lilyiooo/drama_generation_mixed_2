"""Full-outline review and rewrite, adapted from the independent demo."""
REVIEW_PROMPT = """你是全剧叙事审校编辑。请通读以下本轮待审校的全部逐集大纲，评审叙事一致性并提出可执行的问题清单。本轮只评审并评分，不重写大纲。
故事基本信息：
{{Background}}
总集数：{{Total}}
本轮待审校的完整逐集大纲：
...
评分要求：
给当前输入的完整大纲打一个0—100分的总分total_score（数值），并在overall_assessment中说明主要扣分依据。每次使用相同标准，重点评价叙事一致性、人物与世界状态连续性、事件因果和跨集衔接，并综合考虑主线节奏、伏笔铺垫与回收及故事设定的遵循程度。
90—100：整体严密连贯，无明显关键矛盾，仅有少量可优化处；75—89：整体连贯，存在少量局部衔接或铺垫问题；60—74：存在多处明显问题，影响部分主线或人物逻辑；40—59：重要因果或人物状态反复冲突，需较大调整；0—39：核心叙事严重断裂，难以形成自洽故事。各区间内按问题严重程度、数量和影响范围细分。只评价当前文本，不推测修改轮次或默认修订版更好。
...
只输出JSON对象：
{
 ...
 "revision_strategy": "如何统筹{{Total}}集的因果、节奏、关键状态和伏笔回收"
}
没有发现问题时issues输出空数组。severity仅使用严重、一般、建议。
"""

REWRITE_PROMPT = """你是全剧大纲修订编剧。依据评审意见和本轮待修订的完整逐集大纲，重新生成校准后的完整{{Total}}集逐集大纲。
故事基本信息：
{{Background}}
本轮待修订的完整逐集大纲：
{{Original}}
评审意见（情节概述与建议请结合上方完整大纲核对，总分仅供参考）：
{{Review}}
