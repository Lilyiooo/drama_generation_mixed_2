...
class ShortAnalyze(BaseLlmOperation):
    ...
    def ip_part_expand(self,maincall=""):
        ...
        for partid, partname in enumerate(self.target_parts):  
            ...
            #}
            # type 14:# {num_episode} 集，每集 {min_perepisode} 
            print("in part expand self.IPdata.outline_plan", self.IPdata.outline_plan)
            
            part_plan = self.IPdata.outline_plan_str if len(self.IPdata.outline_plan[partname]) < 5 else self.IPdata.outline_plan[partname]
            if isinstance(self.IPdata.outline_plan, dict) and partname in self.IPdata.outline_plan:
                logger.debug(f"using outline_plan empty {partname}, {self.IPdata.outline_plan[partname]}, using total plan：{self.IPdata.outline_plan_str}")
            variables = {
                "partname": partname, 
    ...
    async def ip_part_script_gen(self, datainfo, role_info, world_building):
        ...
        print("scriptgen after restore IPdata",self.IPdata.chapters)
        self.IPdata.heros_str = role_info
        self.IPdata.world_str = world_building
        await self._select_part_text()
        ...
        for partid, partname in enumerate(self.target_parts):
            ...
            print("scriptgen input plan", episode_plans)
            # 5 prm
            print("input partscript:", partname, text5w[:20],episode_core_story[:20])
